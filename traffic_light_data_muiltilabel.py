...
class TrafficLightDataLayer(caffe.Layer):
    ...
    def setup(self,bottom,top):
        ...
        self.num_class1 = params['num_class1']
        self.num_class2 = params['num_class2']              #the number of classes

        top[0].reshape(
            self.batch_size,3,params['crop_size'][1],params['crop_size'][0]
        )
        top[1].reshape(self.batch_size,1,self.num_labels)
        top[2].reshape(self.batch_size,1,self.num_labels)

    def forward(self,bottom,top):

        for idx in range(self.batch_size):
            img,label1,label2 = self.batch_loader.load_next_image()
            if img is None:
                continue
            top[0].data[idx,...] = img
            top[1].data[idx,...] = label1
            top[2].data[idx,...] = label2
...
class BatchLoader(object):
    ...
    def __init__(self,params,result):
        ...
        self.num_class1 = params['num_class1']       #the total number of all kinds of class1
        self.num_class2 = params['num_class2']       #the total number of all kinds of class2
    ...
    def load_next_image(self):
        ...
        img_path, labels1, labels2 = self.data_lines[self._cur].split('#')
        img = cv2.imread(img_path)
        if img is None:
            print '{} has problem'.format(img_path)
            return None,None
        img = cv2.resize(img,self.crop_size)
        label1_array = np.zeros((1,self.num_labels))
        label2_array = np.zeros((1,self.num_labels))
        #img = self.transformer.preprocess(img,degree=self.degree,crop_size=self.crop_size)
        labels1 = map(int,labels1.split())
        labels2 = map(int,labels2.split())
        for idx,(label1,label2) in enumerate(zip(labels1,labels2)):
            label1_array[0,idx] = label1
            label2_array[0,idx] = label2
        img = np.transpose(img,(2,0,1))
        self._cur += 1
        return img,label1_array,label2_array